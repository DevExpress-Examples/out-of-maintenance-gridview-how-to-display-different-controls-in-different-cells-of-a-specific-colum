...
public class NorthwindDataProvider {
    ...
    public static void UpdateProductUnitPrice(int productID, decimal unitPrice) {
        using (OleDbConnection connection = new OleDbConnection(WebConfigurationManager.ConnectionStrings["Northwind"].ConnectionString)) {
            OleDbCommand updateCommand = new OleDbCommand("UPDATE [Products] SET [UnitPrice] = ? WHERE [ProductID] = ?", connection);
